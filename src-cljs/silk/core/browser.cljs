...
(defn- elements [] (.getElementsByTagName js/document "*"))

(defn- elementsWithAttribute
  [k]
  (filter #(.getAttribute % (name k)) (cr/as-seq (elements))))

(defn spin-component-with-data
  "Splices data with component with given id"
  [id data]
  (let [template (elementById id)
...
(defn spin-components
  ...
  []
  (doseq [el (elementsWithAttribute :data-sw-source)]
    (go
